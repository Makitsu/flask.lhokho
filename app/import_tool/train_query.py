...
def find_ticket(date,departure_station_name,arrivals_name):
    departure_station = models.Station.from_name(departure_station_name)
    departure_connections = []
    for name in arrivals_name:
        departure_connections.append(models.Station.from_name(name))
    segment_details = True

    q = queue.Queue()
    threads = [threading.Thread(target=thread_fares,
                                args=(date, departure_station.code_tl, connection, passengers, segment_details, q)) for
               connection in departure_connections]
    for th in threads:
        th.start()
        time.sleep(10)
    ...
    for i in range(q.qsize()):
        ticket_df = q.get()
        send_notification(ticket_df)

def main():
    #test connections
    date = '2020-05-15T09:00:00UTC'
    departure_station = models.Station.from_name(153)
